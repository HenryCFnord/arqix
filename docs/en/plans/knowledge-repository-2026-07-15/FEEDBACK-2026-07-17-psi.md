...
- FR-A3 generalizes the report-unit machinery into configurable projections; the fixed-column catalog landed as the source-catalog unit under the drift gate (US-08-01-37), the configurable column set stays open.
...
- FR-C2 and FR-C3 are the layered-ontology core: project-defined, checker-validated properties and namespaces; the validation half exists — a project property document with declared rdfs.domain/range gets ONT-001/003/007 resolution and contract checks (US-08-01-36), the namespace half landed as the FM-010 placement contract — the dir-template rendered back from each document's own properties (US-08-01-38); per-context configuration beyond placement (id prefixes, allowed kinds) stays open.
