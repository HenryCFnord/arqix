...
- [x] B1 data side: US-08-01-41 + REQ-08-01-41-01/-02, spec-first red/green; `report claims` as the second gated export and the evidence-coverage unit (Q-12) — the B-slices' remaining open item is the provenance layer (review records, PROV orientation), a design conversation.
...
- [x] Module vocabularies as shipped data: ADR-0021 accepted 2026-07-19 (owner decisions: embedded in the binary from this corpus's ontology documents, precedence corpus > module with the reserved core guarded, bindings module-fixed); REQ-08-01-31-02/-03 on US-08-01-31, spec-first red/green — the effective ontology resolves core, module, and corpus layers, a corpus without docs/ontology is valid, deselecting a module removes its vocabulary, and ONT-009 reports divergent core redefinitions (identical re-declaration stays silent, so this authoring corpus needs no special case).
  The catalog rows for ONT-007/008 (missed when the catalog was written) land alongside ONT-009.
- [x] D6 entities and crosswalks: ADR-0022 accepted 2026-07-19 (owner decisions: entity as its own document kind — the P1 answer, with ADR-0007 untouched because the entity node is a document node; arqix mapping properties in the knowledge-base module; ONT-010 closes the silent-external-predicate hole); US-08-01-43 + REQ-08-01-43-01/-02/-03, spec-first red/green — the module-shipped entity/describes/mapping vocabulary, the declared external-property list, and the crosswalk unit (Q-13) as the twelfth gated unit.
- [x] First knowledge content on the new machinery: ENT-0001 (RFC 2119) with SRC-0002 describing it and the exact-match onto ietf:rfc2119 as the crosswalk's first row; the rule catalog carries the first two claims (keyword subset from SRC-0002, EARS patterns from SRC-0003) with CLM-0001 as the first claim record (representation digest-pinned, unreviewed until the owner sets a verdict) — every layer of ADR-0018/0019/0022 now has real corpus data behind it.
