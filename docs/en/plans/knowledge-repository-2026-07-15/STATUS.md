...
- [x] FR-E1: REQ-04-01-10-03 on US-04-01-10 (the canonical diagnostics story), spec-first red/green — `lint frontmatter`, `lint requirements`, and `trace markers` answer `--format json` with the shared diagnostics payload; the unversioned findings/summary shape inherited from the retired reference checkers is gone, text output and exit codes stay byte-identical, `trace markers` keeps its coverage counters as additive keys; the ICD diagnostics unit records the convergence.
