...
7. [ ] **Marker freshness** (V&V — arqix turned on its own architecture, validating it the way an IVVQ process validates a system rather than adding command surface): a `verifies`/`implements` marker still resolves after the code beneath it changes, so coverage can count a requirement as verified on a stale marker — close the gap with git arithmetic rather than code analysis, flagging a marker as possibly-stale when its file changed after the linked requirement or story last moved, so `trace coverage` and `verify` measure current verification and not historical marker placement.
8. [ ] **Derivation checks** (V&V — the `derived-from` promise, gated): the embedded Mermaid views carry a `<!-- derived from workspace.dsl -->` comment that nothing enforces, and the C4 audit found exactly that drift by hand — fold the structurizr→Mermaid pipeline (the open arc42 chapter-11 item) into `verify` so a committed diagram that diverges from the model fails the gate, turning a declared document↔model link into a checked one the same way `trace check` does for US→REQ→code.
