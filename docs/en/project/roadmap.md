...
Progress through the implementation phases is measured by one number: the share of functional requirements referenced by `arqix:verifies` markers in the test suite (reported by `scripts/check_trace_markers.py`; currently 68/126 — the generated [scoreboard](../reports/units/scoreboard.md) is the always-current view).
