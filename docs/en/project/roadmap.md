...
Progress through the implementation phases is measured by one number: the share of functional requirements referenced by `arqix:verifies` markers in the test suite (reported by `arqix trace markers`; currently 107/151 — the generated [scoreboard](../reports/units/scoreboard.md) is the always-current view).
...
5. [ ] **Render & languages** (`render pdf`, language-aware site, DE translations).
   The `render pdf` plumbing landed — containerised Pandoc/eisvogel via `just render-pdf`, the C4 SVGs embedded — but three quality gaps must close before the release: the embedded SVGs render broken and need a faithful vector path, the render bundles the whole corpus into one artefact where it should produce one PDF per document (which revisits the one-artefact-per-package assumption in ADR-0013), and it needs a proper title page.
