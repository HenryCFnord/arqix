...
| [ADR-0012](../adr/ADR-0012-id-policy-model.md) | ID policy model: per-family ID patterns with named groups; the trace graph derives owner-story, cross-cutting domain, and sequencing from the configured groups; defaults reproduce the current shapes | accepted |
