...
| [ADR-0021](../../adr/ADR-0021-module-vocabularies-as-shipped-data.md) | Module vocabularies as shipped data: modules embed their ontology documents in the binary at compile time (the authoring corpus stays the single source); the effective ontology layers corpus over modules over core with ONT-009 guarding divergent core redefinitions; a corpus without `docs/ontology` is valid; rule bindings stay module-fixed | accepted |
| [ADR-0022](../../adr/ADR-0022-entities-and-crosswalks.md) | Entities and crosswalks: a domain entity is a document of its own kind (the one identity rule survives — the entity node is a document node), descriptions attach via `describes`; the knowledge-base module carries the SKOS-inspired mapping properties (`maps-to` plus four refinements) targeting external IRIs; undeclared external predicates are ONT-010; the crosswalk is a gated report unit | accepted |
