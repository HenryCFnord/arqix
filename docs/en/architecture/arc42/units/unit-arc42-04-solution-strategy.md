...
5. **One parser, two layers.** Documents are parsed exactly once by a shared Document Parser: a lossless concrete-syntax layer that the formatter needs for meaning-preserving rewrites (REQ-01-01-03-03), and a semantic document model (frontmatter, sections and anchors, directives, trace markers) that store, linter, assembler, and trace engine consume (REQ-02-01-09-*, REQ-05-01-10-*). One grammar, one implementation — parser drift across components would silently break determinism.
6. **Delegated rendering.** Pandoc and site toolchains stay external processes behind an orchestrator; arqix owns inputs, artefact placement, and error transparency, never the renderer (REQ-04-01-03-*).
