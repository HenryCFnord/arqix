...
The parsers live in `src/trace.rs` (markers, directives) and `src/parser.rs` (frontmatter triples); the oracle `scripts/arqix_trace.py` is the conformance reference.
