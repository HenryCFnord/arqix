...
2. **Diagnostics projection (`trace coverage`).** A coverage gap is a diagnostic in the tool-wide findings format — severity, stable code, message, source location (REQ-00-00-00-03) — plus a summary by requirement kind. Agents and CI parse one findings format across the whole tool. Stable codes:
   - `TRC-COV-001` (error): a functional requirement has no `verifies` marker.
